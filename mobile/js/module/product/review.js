            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.comment != undefined && data.comment.length != undefined) {
                        ...
                        for (var i=0; data.comment.length > i; i++) {
                            ...
                            if (data.comment[i]['comment_reply_css'] == undefined) {
                                ...
                                aHtml.push('<span class="grade '+data.use_point+'"><img src="http://img.echosting.cafe24.com/skin/mobile_ko_KR/board/ico_star'+data.comment[i]['comment_point_count']+'.png" alt="'+data.comment[i]['comment_point_count']+'점" /></span>');
                                aHtml.push('</div>');
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('<div class="captcha ' +data.comment_write['use_captcha']+ '"><span class="img"></span><div class="form">' +data.comment_write['captcha_image']+data.comment_write['captcha']+ '<p>왼쪽의 문자를 공백없이 입력하세요.<br>(대소문자구분)</p></div></div>');
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<div class="captcha ' +data.comment_reply['use_captcha']+ '"><span class="img"></span><div class="form">' +data.comment_reply['captcha_image']+data.comment_reply['captcha']+ '<p>왼쪽의 문자를 공백없이 입력하세요.<br>(대소문자구분)</p></div></div>');
